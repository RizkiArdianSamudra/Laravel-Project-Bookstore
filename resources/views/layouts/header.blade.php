...
<div class="sticky top-0 bg-[#27ae60] border-b-2 border-black/50">
    <nav class="flex justify-between items-center px-20 py-3">
        ...
        <div class="flex items-center">
            <a href="/" class="navLink{{ Request::is('/') ? ' aktif' : '' }} px-4 py-1 text-white hover:underline rounded-full ml-10">Home</a>
            @if(Route::current()->getName() == 'register' || Route::current()->getName() == 'home')
                <a href="/register" class="navLink{{ Request::is('register') ? ' aktif' : '' }} px-4 py-1 text-white hover:underline rounded-full ml-10">Register</a>
            @elseif(Route::current()->getName() == 'login')
                <a href="/login" class="navLink{{ Request::is('login') ? ' aktif' : '' }} px-4 py-1 text-white hover:underline rounded-full ml-10">Login</a>
            @endif
        </div>
    </nav>
</div>
