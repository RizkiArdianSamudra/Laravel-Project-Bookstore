        ...
        <div class="flex min-h-[36rem] rounded-3xl border-2 border-[#27ae60]/50 shadow-lg">
            ...
            <div class="w-3/4 px-10 py-6">
                <div class="flex justify-between">
                    <h1 class="font-bold text-2xl">Daftar Buku</h1>
                ...
                <div class="mt-5 grid grid-cols-5 gap-[2px]">
                    <p class="px-4 py-2 font-medium bg-[#27ae60]/50 rounded-tl-lg">Nama Buku</p>
                    <p class="px-4 py-2 font-medium bg-[#27ae60]/50">Stok</p>
                    <p class="px-4 py-2 font-medium bg-[#27ae60]/50">Harga</p>
                    <p class="px-4 py-2 font-medium bg-[#27ae60]/50">Deskripsi</p>
                    <p class="px-4 py-2 font-medium bg-[#27ae60]/50 rounded-tr-lg">Tindakan</p>
                </div>

                @foreach ($buku as $item)
                    <div class="grid grid-cols-5 gap-[2px] h-11">
                        <p class="px-4 py-2 border-b-[1px] border-black/50">{{ $item->nama_buku }}</p>
                        <p class="px-4 py-2 border-b-[1px] border-black/50">{{ $item->stok_buku }}</p>
                        <p class="px-4 py-2 border-b-[1px] border-black/50">{{ $item->harga_buku }}</p>
                        <p class="px-4 py-2 border-b-[1px] border-black/50 overflow-x-auto">{{ $item->deskripsi }}</p>

                        <div class="px-4 border-b-[1px] border-black/50 flex justify-center items-center space-x-4">
                            <form action="/admin/delete-buku/{{ $item->id_buku }}" method="POST">
                                @csrf
                                @method('delete')
                            ...
                            <form action="/admin/edit-buku/{{ $item->id_buku }}" method="GET">
                                @csrf
                                <button type="submit">
